...
if settings.startup['rpm_entity'].value ~= 30 or settings.startup['rpm_items'].value ~= 30 or settings.startup['overload'].value ~= 0 then
    ...
    --These types (data.raw[entity_type]) are placeable entities trimmed down to potentially expensive ones
    local entity_types = {'accumulator','artillery-turret','beacon','boiler','burner-generator','character','container','logistic-container','infinity-container','assembling-machine','rocket-silo','furnace','electric-energy-interface','unit-spawner','combat-robot','construction-robot','logistic-robot','generator','heat-interface','lab','linked-container','market','mining-drill','offshore-pump','infinity-pipe','player-port','radar','reactor','roboport','simple-entity','simple-entity-with-owner','simple-entity-with-force','solar-panel','spider-leg','storage-tank','linked-belt','tree','turret','ammo-turret','electric-turret','fluid-turret','unit','car','artillery-wagon','cargo-wagon','fluid-wagon','locomotive','spider-vehicle'}
