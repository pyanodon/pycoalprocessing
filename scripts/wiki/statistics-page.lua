...
local function update_statistics_page(gui, player, include_laggy_calculations)
    ...
    if include_laggy_calculations then
        add_statistic(gui,
            { "pywiki-statistics.tech-tree-completion", statistics.tech_tree_completion })
    end
    add_statistic(gui, { "pywiki-statistics.playtime", statistics.hour, statistics.minute, statistics.second })
    add_statistic(gui,
        { "pywiki-statistics.time-of-day", statistics.daytime_hour, statistics.daytime_minute, statistics.am_pm })
    add_statistic(gui, { "pywiki-statistics.mods-installed", statistics.active_mods })
    if include_laggy_calculations then
        add_statistic(gui,
            { "pywiki-statistics.buildings-placed", statistics.buildings_placed })
    end
    if include_laggy_calculations then
        add_statistic(gui,
            { "pywiki-statistics.recipes-unlocked", statistics.recipes_unlocked })
    end
...
py.register_on_nth_tick(60, "wiki", "pycp", function()
