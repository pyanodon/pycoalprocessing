...
local function on_search(search_key, gui)
    ...
    for _, row in pairs(gui.children) do
        local visible = row.name:find(search_key, 1, true) or row.name:find(search_key:gsub(" ", "-"), 1, true) or
            row.name:find(search_key:gsub(" ", ""), 1, true)
        ...
        if not tech.hidden and not tech.research_unit_count_formula and tech.name ~= "fluid-handling" and tech.name ~= "plastics" then
            for _, effect in pairs(tech.effects) do
                if effect.type == "unlock-recipe" then
                    ...
                    recipe_required_science[ name ] = min(recipe_required_science[ name ] or math.huge,
                        calculate_highest_science_pack(tech))
                end
            end
        end
    end
    ...
    for name, fluid in pairs(prototypes.fluid) do
        if not fluid.hidden and not (script.active_mods[ "pyalternativeenergy" ] and fluid.name == "combustion-mixture1") then
            ...
            if fluid.name == "neutron" then
                voidable =
                "[entity=neutron-absorber-mk01] [entity=neutron-absorber-mk02] [entity=neutron-absorber-mk03] [entity=neutron-absorber-mk04]"
                voidable_elem_tooltip = { type = "entity", name = "neutron-absorber-mk01" }
            elseif script.active_mods[ "pyindustry" ] then
                local void_entity_name = (fluid.default_temperature or 15) < (fluid.gas_temperature or math.huge) and
                    "py-sinkhole" or "py-gas-vent"
    ...
    for name, item in pairs(prototypes.item) do
        if item.fuel_category and not item.hidden then
            ...
            table.insert(storage.solid_fuel_spreadsheet_data.rows, {
                ...
                [ "unlocked-at" ] = unlocked_at,
                search_key = name ..
                    "|" .. item.fuel_category .. "|" .. (burnt_result_order or "") .. "|" .. unlocked_at.pack
            })
        end
    end
end
...
local function generate_decay_spreadsheet_data(required_science)
    ...
    for name, item in pairs(prototypes.item) do
        ...
        elseif total_decay_ticks > decay_ticks then
            decay_time_string = decay_time_string ..
                " [font=default-small](" .. py.format_large_time(total_decay_ticks / 60) .. ")[/font]"
        end
        ...
        table.insert(storage.decay_spreadsheet_data.rows, {
            ...
            [ "decay-result" ] = {
                ...
                order = string.format("%06x", #decay_chain) ..
                    string.format("%06x", table_size(already_seen)) .. decay_result
            },
            [ "unlocked-at" ] = unlocked_at,
